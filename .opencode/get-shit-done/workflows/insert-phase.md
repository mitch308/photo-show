...
```bash
INIT=$(node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
