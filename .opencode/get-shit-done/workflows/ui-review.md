...
```bash
UI_AUDITOR_MODEL=$(node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read C:/workspace/photo-show/.opencode/agents/gsd-ui-auditor.md for instructions.
...
```bash
node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
