...
```bash
INIT=$(node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" init quick "$DESCRIPTION")
...
```bash
node "C:/workspace/photo-show/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
