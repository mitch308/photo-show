...
- **Global scope**: `C:/workspace/photo-show/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/workspace/photo-show/.opencode/notes/`)
...
2. Glob `C:/workspace/photo-show/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (C:/workspace/photo-show/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/workspace/photo-show/.opencode/notes/` — works in any directory
